...
{
	glm::vec3 candidates[500]; // Allocate space for candidates
	...
	for (int i = 0; i < NV; i++)
		for (int j = 0; j < NV; j++)
			...
			{    
                ...
                glm::vec3 cellSize = glm::vec3(cellSizX, cellSizY, cellSizZ);
				glm::vec3 coord = indexToCoordCPU(index, min, cellSize);
