...
vec4 candidates[500];    // List of candidates
...
vec3 getRealSunset(in vec3 rd )
{
    ...
    col += vec3(.6, .6 - exp( -rd.y*20. )*.3, .0) * exp(-rd.y*9); // Red / Green 
    col += vec3(.6, .7, 0.8) * (1. - exp(-rd.y*8.) ) * exp(-rd.y*.2) ; // Blue
    
    col += vec3(1.0, .8, .55) * pow( max(dot(rd,-iSunDir),0), 30.) * .3; // Sun
    col += pow(max(dot(rd, -iSunDir),0.), 150.0) *.15;
...
vec4 renderMountains(in vec3 ro, in vec3 rd)
{
	...
	else
	{
		...
		// fog
		float fo = 1.0 - exp(-pow(0.001*t / (SC * 4), 1.5));
	...
	for (int i = 0; i < n; i++)
	{
		...
		while (tIn <= tOut) // Iterate sphere
		{
			...
			if (!bFlat || (pos.y > iVoxMin.y + precLight * 1.4)) // Condition for flat cloud
			{
				...
				localDen = exp(-distance(pos, cloudPos) / (radius*(0.8 + 0.2*den)));
...
vec4 render(in vec3 rayOrg, in vec3 rayDir, in vec2 px, in vec4 resM)
{
	...
	if (iTurn == 0) // Morning scene
	{
		...
		// sun glare    
		skycol += vec4(getMorningSky(rdNorm.xyz) + 0.2*sunColor*exp(-sun), 0);
    ...
    float fexp = exp(-sun * 1000.0 / sunSize);
		float fexp2 = exp(-sun * 1000.0 / 0.1);
    skycol = vec4(0.8*sunColor*fexp + 0.9*sunColor*fexp2, 1.0);
		skycol += vec4(getRealSunset(rdNorm.xyz) + stars,0);	
