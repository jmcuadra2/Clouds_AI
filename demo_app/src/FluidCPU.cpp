...
FluidCPU::FluidCPU(int dimM, int dimN, int dimO, float dt, float diff, float visc)
{
	this->dimM = dimM;
	this->dimN = dimN;
	this->dimO = dimO;
...
//JMCT
/**
 * @brief Initialize array pointers for previuos values of 3D fluid components, for using only in neural net inference
 * 
 * @param force Wind force
 */
void FluidCPU::setPrevForceSource(float force)
{
    for(int i = 0; i < dimM + 2; i++) {
        for(int j = 0; j < dimN + 2; j++) {
            for(int k = 0; k < dimO + 2; k++) {
                uPrev[IX(i,j,k)] = force;
                vPrev[IX(i,j,k)] = 0.0f;
                wPrev[IX(i,j,k)] = 0.0f;               
            }
