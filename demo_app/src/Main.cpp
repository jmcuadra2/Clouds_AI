...
void initGL(int argc, char* argv[])
{
	...
	std::string title = "Real-time ontological cloud modeling -- UNED Thesis. Rendering at " + std::to_string(SCR_W) + " X " + std::to_string(SCR_H) + ".";
...
void applyWind() // Apply wind force
{
	...
	if (parallel)
	{
		windGridCUDA.clearData(); // Clear fluid internal data
	...
	if (onPlay)
	{
		...
		if (firstPass) // If first iteration
		{
			...
			if (precomputeTimeOut >= PRECOMPTIMEOUT) // Check for regular precompute light (shading)
			{
				...
				std::cout << "PRECOMPUTING LIGHT TIME =" << msElapsed / CLOCKS_PER_SEC << std::endl;
			}
...
void mouseWheel(int button, int dir, int x, int y)
{
	...
	std::cout << "Depth-1 = " << cloudDepth << std::endl;
}
