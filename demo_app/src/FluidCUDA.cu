...
Clear pre-data in device
*/
__global__ void clearUVWDev_One(float x[][N + 2][O + 2])
{
...
void FluidCUDA::clearUVW()
{
	...
	clearUVWDev << <block, thread >> > (devU, devV, devW);
	clearUVWDev << <block, thread >> > (devUy, devVy, devWy);
	clearUVWDev_One << <block, thread >> > (devUPrevy);
...
{
	cudaMemcpy(devUPrev, u, size * sizeof(float), cudaMemcpyHostToDevice);
	cudaMemcpy(devVPrev, v, size * sizeof(float), cudaMemcpyHostToDevice);
	cudaMemcpy(devWPrev, w, size * sizeof(float), cudaMemcpyHostToDevice);
}
